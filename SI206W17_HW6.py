...
## There are tests for each problem; you should also follow the instructions for things to do in the code that cannot be tested with unit tests,
# e.g. using certain structures.
...
## You can test this out with any code you like here, and similar below other problems,
# but make sure to comment out any code that uses up the iterator in order to pass the tests!
print("begin")
sen_dict = {}
pro_dict = {}
for i in range(0,5):
    sen_dict[names[i]] = seniority[i]
    pro_dict[names[i]] = programs_written[i]

for i in student_tups_list:
    print(i[1] == sen_dict[i[0]])
    print(i[2] == pro_dict[i[0]])
print("end")
...
programmers = [Student(i[0], i[1], i[2]) for i in student_tups_list]
...
# in order to create an map instance iterator of numbers representing the productivity of each student.
...
prod_iter = map(lambda i: programs_written[i] / seniority[i], range(0,5))
...
for i in range(0,5):
    names_and_productivities.append((names[i], prod_list[i]))
...
## There is a test for this but an even more fun test is to uncomment the code below which invokes the main_filterer
# function and prints each line from the generator without blank lines in between (that's what the comma is doing).
